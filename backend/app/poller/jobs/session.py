...
Multi-user: every function here works on one `GatewayRuntime`. The monitor walks
all of them but keeps each one's failures and state strictly to itself — one
user's dead gateway must never release another user's live session.
"""
...
async def _detect_account(runtime: GatewayRuntime) -> str | None:
    if runtime.config.account_id:
        return runtime.config.account_id
    try:
        accounts = await runtime.client.iserver_accounts()
...
async def _persist_account(runtime: GatewayRuntime, account_id: str) -> None:
    """Register the account and give a brand-new one its own settings row.

    The label comes from the gateway config so the UI switcher has something to
    show the moment a new user first logs in.
    """
    ...
    async with AsyncSessionLocal() as session:
        stmt = (
            pg_insert(Account)
            .values(account_id=account_id, label=runtime.label)
            .on_conflict_do_nothing(index_elements=["account_id"])
        )
        await session.execute(stmt)

        # Backfill a label onto an account first seen before it had one.
        existing = await repo.account_by_id(session, account_id)
        if existing is not None and not existing.label:
            existing.label = runtime.label

        # A fresh user starts with an empty watchlist and the default
        # thresholds — never a copy of another user's tracked underlyings.
        await session.execute(
            pg_insert(AccountSetting)
            .values(
                account_id=account_id,
                data={"underlyings": [], "alerts": dict(DEFAULT_SETTINGS["alerts"])},
            )
            .on_conflict_do_nothing(index_elements=["account_id"])
        )
...
async def monitor() -> None:
    """Run the passive gatekeeper over every gateway, one user at a time.

    Each runtime is isolated: an exception while checking one user's gateway
    must not skip the others, and no user's state is ever read or written while
    handling another's.
    """
    for runtime in all_runtimes():
        try:
            await monitor_one(runtime)
        except Exception as exc:
            log.warning(
                "monitor_gateway_failed",
                gateway=runtime.gateway_id, error=str(exc),
            )


async def monitor_one(runtime: GatewayRuntime) -> None:
    """Passive gatekeeper for one gateway: checks auth_status, releases strays.
    ...
    changed = False
    try:
        status = await runtime.client.auth_status()
        authenticated = bool(status.get("authenticated"))
        connected = bool(status.get("connected"))
        competing = bool(status.get("competing"))

        login_in_progress = state.status in (
        ...
        if authenticated and not state.user_logged_in:
            if _recent_login_intent(state):
                ...
                log.info("monitor_adopting_late_login", gateway=runtime.gateway_id)
                try:
                    await _finalize_authenticated_session(runtime)
                except Exception as exc:
                    log.warning(
                        "monitor_adopt_failed",
                        gateway=runtime.gateway_id, error=str(exc),
                    )
            ...
            try:
                ...
                log.info(
                    "monitor_logged_out_stray_session", gateway=runtime.gateway_id
                )
            except (IBKRError, IBKRAuthError):
                pass
        ...
        elif competing and not state.user_logged_in:
            changed = state.update(
                status=GatewayStatus.DISCONNECTED,
                authenticated=False,
                competing=True,
        ...
        elif state.user_logged_in and not authenticated:
            changed = state.update(
                status=GatewayStatus.DISCONNECTED,
                authenticated=False,
                connected=connected,
        ...
        log.warning(
            "monitor_unexpected_error", gateway=runtime.gateway_id, error=str(exc)
        )
        if not state.user_logged_in:
            changed = state.update(
                status=GatewayStatus.DISCONNECTED,
    ...
    if changed:
        log.info("session_state_changed", **state.to_dict())
        await broadcast_session(state)
...
def _recent_login_intent(
    state: SessionState, window_seconds: int = _LOGIN_ADOPT_WINDOW_SECONDS
) -> bool:
    """True if this user clicked "Pull Fresh Data" within `window_seconds`."""
    ts = state.login_requested_at
    if not ts:
        return False
    try:
...
async def _await_authentication(runtime: GatewayRuntime, timeout: float) -> bool:
    ...
    while loop.time() < deadline:
        try:
            ...
            if cur != last_logged:
                log.info("auth_poll", gateway=runtime.gateway_id, **cur)
                last_logged = cur
            ...
            log.warning("auth_poll_error", gateway=runtime.gateway_id, error=str(exc))
...
async def _finalize_authenticated_session(runtime: GatewayRuntime) -> dict:
    ...
    account_id = await _detect_account(runtime)
    if not account_id:
        state.update(
            status=GatewayStatus.DISCONNECTED,
            authenticated=False,
    ...
    await _persist_account(runtime, account_id)
    state.update(
    ...
    # Flex is per-user and optional — a user who hasn't set up a Flex query in
    # IBKR Account Management simply skips this.
    flex_token = runtime.config.flex_token
    flex_query = runtime.config.flex_query_id
    if flex_token and flex_query:
        try:
            from app.clients.ibkr.flex_web import fetch_flex_trades
            log.info("flex_web_request", gateway=runtime.gateway_id)
            ...
            if flex_trades:
                ...
                log.info(
                    "flex_web_import", gateway=runtime.gateway_id,
                    parsed=len(flex_trades), inserted=flex_count,
                )
        except Exception as exc:
            log.warning("flex_web_failed", gateway=runtime.gateway_id, error=str(exc))
    ...
    for resource in ("positions", "account", "trades"):
        await broadcast_event(resource, account_id)
    for resource in ("market", "signals"):
...
async def orchestrate_login(runtime: GatewayRuntime) -> dict:
    ...
    try:
        ...
        if not await _await_authentication(runtime, timeout):
            state.update(
                status=GatewayStatus.DISCONNECTED,
                authenticated=False,
        ...
        return await _finalize_authenticated_session(runtime)

    except Exception as exc:
        log.error("orchestrate_login_failed", gateway=runtime.gateway_id, error=str(exc))
        state.update(
            status=GatewayStatus.DISCONNECTED,
            authenticated=False,
...
async def orchestrate_logout(runtime: GatewayRuntime) -> dict:
    """User-initiated logout: release this user's session for IBKR mobile."""
    try:
        await runtime.client.logout()
        log.info("orchestrate_logout_ok", gateway=runtime.gateway_id)
    except (IBKRError, IBKRAuthError):
        log.warning("logout_call_failed_continuing", gateway=runtime.gateway_id)
    except Exception as exc:
        log.error("logout_unexpected", gateway=runtime.gateway_id, error=str(exc))
    ...
    await broadcast_session(runtime.state)
